...
import { generateCaptions, moderateImage } from "@/lib/ai.functions";
...
export default function Create() {
  ...
  async function openAi() {
    ...
    try {
      ...
      const { captions } = await generateCaptions({ data: { imageDataUrl: dataUrl } });
  ...
  async function submit() {
    ...
    try {
      const dataUrl = await fileToScaledDataUrl(file);
      const result = await moderateImage({ data: { imageDataUrl: dataUrl } });
      if (result.severe) {
        setPosting(false);
