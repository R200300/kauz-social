...
/** Heartbeat: stamp the current user's last_seen. */
...
export async function fetchConversations(userId: string): Promise<Conversation[]> {
  ...
  return partnerIds
    .map((id) => {
      ...
      const partner =
        profileMap.get(id) ??
        ({ id, username: null, full_name: null, avatar_url: null, last_seen: null } as ChatUser);
...
/** Mark every message from `partnerId` to `userId` as read. */
...
/** Search users by username (to start a new conversation). */
